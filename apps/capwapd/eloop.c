...
int eloop_register_timeout(unsigned int secs, unsigned int usecs,
			   eloop_timeout_handler handler,
			   void *eloop_data, void *user_data, void *node)
{
	...
	{
		struct ap_node *tmp_node = (struct ap_node *)node;
		timeout->ap_id = tmp_node->ap_id;
		timeout->l2tp_id = tmp_node->l2tp_id;
	}
			 ...
			 void *eloop_data, void *user_data, void *node)
{
	...
	while (timeout != NULL) {
		...
		if (timeout->handler == handler &&
		    ...
		    (timeout->user_data == user_data ||
		     user_data == ELOOP_ALL_CTX) && timeout->ap_id == tmp_node->ap_id) {
			if (prev == NULL)
				eloop.timeout = next;
			else
				prev->next = next;
		...
		if (eloop.timeout) {
			...
			if (!os_time_before(&now, &eloop.timeout->time)) {
				...
				tmp->handler(tmp->eloop_data,
					     tmp->user_data,tmp->ap_id,tmp->l2tp_id);
				os_free(tmp);
			}
		}
